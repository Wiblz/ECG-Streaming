...
// Reactive state for accelerometer samples - SvelteMap is reactive when mutated
export const samples = new SvelteMap<string, BufferedAccelerometerSample[]>();
...
export function addSamples(newSamples: BufferedAccelerometerSample[]) {
	...
	for (const [device_id, newDeviceSamples] of byDevice) {
		...
		// Update the map with filtered samples - SvelteMap.set() is reactive
